...
// Usage: node migrations/set_admin_user.js [email]
// Example: node migrations/set_admin_user.js user@example.com
...
if (!existsSync(backendPackageJson)) {
  console.error('❌ Error: Cannot find backend/package.json');
  console.error('Please make sure you are running this script from the project root');
  process.exit(1);
}
...
// Use dynamic import for dotenv since we're mixing ES modules and CommonJS
let dotenv;
try {
  dotenv = require('dotenv');
} catch (err) {
  console.error('❌ Error: Cannot load dotenv package');
...
try {
  ...
  console.error('❌ Error: Cannot load @supabase/supabase-js package');
  console.error('Please run: cd backend && npm install');
  process.exit(1);
}
...
if (!email) {
  ...
  console.error('Usage: node migrations/set_admin_user.js [email]');
  console.error('Example: node migrations/set_admin_user.js user@example.com');
  process.exit(1);
}
